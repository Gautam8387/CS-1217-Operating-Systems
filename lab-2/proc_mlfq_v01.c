...
struct proc *mlfq_queue[MAXPRIORITY+1][NPROC];
int mlfq_queue_size[MAXPRIORITY+1] = {0,0,0};
...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  // // New process created is inserted at the end (tail) of the queue
  // mlfq_queue[np->priority][mlfq_queue_size[np->priority]] = np;
  // mlfq_queue_size[np->priority]++;
...
scheduler(void)
{
  ...
  for(;;){
        ...
        if(p->priority < MAXPRIORITY){
          ...
          for(int i = 0; i < mlfq_queue_size[p->priority]; i++){
            if(mlfq_queue[p->priority][i] == p){
              for(int j = i; j < mlfq_queue_size[p->priority]-1; j++){
    ...
    // /****       MLFQ Scheduler        *****/
    // Each time that the MLFQ algorithm runs, the process at the front of the highest priority non empty queue will be selected to run.
    for(int i = MAXPRIORITY; i >= 0; i--){
      // Each queue is Round Robin, start from 1st element, move everything to the left by 1, and put the 1st element at the end.
      while(mlfq_queue_size[i] > 0){
        p = mlfq_queue[i][0];
        // remove the process from the queue
        for(int j = 0; j < mlfq_queue_size[i]-1; j++){
          mlfq_queue[i][j] = mlfq_queue[i][j+1];
        }
        mlfq_queue_size[i]--;
        ...
        cprintf("RUNNALE process %d is running with priority %d\n",p->pid, p->priority);
        ...
        cprintf("Process %d has budget %d\n\n",p->pid, p->budget);
        // Check if budget is less than or equal to 0
        if(p->budget<=0){
          // delete the process from the queue at that priority level
          for(int k = 0; k < mlfq_queue_size[p->priority]; k++){
            if(mlfq_queue[p->priority][k] == p){
              for(int l = k; l < mlfq_queue_size[p->priority]-1; l++){
                mlfq_queue[p->priority][l] = mlfq_queue[p->priority][l+1];
              }
              mlfq_queue_size[p->priority]--;
              break;
            }
          }
          // Check if priority is greater than 0
          if(p->priority > 0){
            // setpriority(p->pid, p->priority - 1);
            p->priority = p->priority - 1;
            p->budget = DEFAULT_BUDGET;
          ...
          // If budget > 0 then the process will be placed at the tail of the same priority queue.
          for(int i = 0; i < mlfq_queue_size[p->priority]; i++){
            if(mlfq_queue[p->priority][i] == p){
              for(int j = i; j < mlfq_queue_size[p->priority]-1; j++){
                mlfq_queue[p->priority][j] = mlfq_queue[p->priority][j+1];
              ...
              mlfq_queue[p->priority][mlfq_queue_size[p->priority]] = p;
              cprintf("Process %d is at the end of the queue\n", p->pid);
              break;
            }
          }
          p->state = RUNNABLE;
    ...
    // // Original Scheduler
    //  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    //   cprintf("the state of process %d is %d\n",p->pid, p->state);
    //   if(p->state != RUNNABLE)
    //     continue;
    ...
    //   c->proc = p;              
    //   switchuvm(p);                       // the switchuvm(p) will do the switching of the kernel virtual memory to the process's virtual memory.
    //   p->state = RUNNING;                 // Now the process is running, so we can switch to it.
    //                                       // The swtch(&(c->scheduler), p->context)  is the content switch. It will save the current context and load the new context.
    //   swtch(&(c->scheduler), p->context);       
    //   switchkvm();                        // The switchkvm function will switch the process's virtual memory to the kernel virtual memory.
...
} // In above the scheduling is done by the policy of round robin. The scheduler will loop over the process table and find the process that is in the RUNNABLE state. If it finds one, it will switch to that process. The process will run until it calls the yield function or it is blocked. When the process is done running, it will call the yield function and the scheduler will switch to another process. 
...
wakeup1(void *chan)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state == SLEEPING && p->chan == chan){
      p->state = RUNNABLE;
      // add new process at the head of old_priority queue
      // mlfq_queue_size[p->original_priority]++;
      // for(int i = mlfq_queue_size[p->original_priority]; i > 0; i--){
      //   mlfq_queue[p->original_priority][i] = mlfq_queue[p->original_priority][i-1];
      // }
      // mlfq_queue[p->original_priority][0] = p;
...
settickets(int numTickets)
{
  ...
  if(numTickets < 0 || numTickets > MAXTICKET){
